...
- true (what you have): If no item is highlighted, it will automatically select the first
  item in the completion list
- false: If no item is highlighted, pressing <C-y> does nothing (no completion is
  inserted)
...
This is a very convenient setup that ensures <C-y> always does something useful, even
if you haven't explicitly navigated to a completion item.
...
-  If the argument is a file or symlink, it uses your getTrueName script to get the directory path and changes to that
-  Otherwise, it tries to change to the argument as-is (which will show an error if invalid)
