...
2025-08-23 補充：目前个理解就是，用 `:Mason` 是裝佇 `~/.local/share/nvim/mason`
內底，干焦 nvim 用會著；用 Homebrew/npm 去裝，shell 環境攏會用得。總是，兩種
攏是手動法，若將 `~/.config/nvim` repo 複製去別台機器，即兩種法度所安裝个
linter 就愛閣手動安裝一擺。所以頭前毋才有 mason-null-ls 佮 mason-tool-installer
遐个設定，按呢就免手動安裝，全新進入 nvim 伊就會家己掠來裝甲好勢。
...
2025-08-23 補充：頂面內底有出現真濟 keymaps for textobjects，我著愛好好仔利用
一下！這是 tree-sitter 所管轄个專門个 syntax parser 才有才調選會著个 textobject。

發現 `ensure_installed` list 內底 `markdown`, `markdown_inline` 攏重複出現，
這就表示某所在無用 require('astrocore').list_insert_unique() 去做插入元素所致。

原來就是 Warp Ai 寫 treesitter-markdown.lua 寫了無好，伊用內建个 vim.tbl_extend，
無用 astrocore 另外設个 list_insert_unique。tbl_extend 若是處理 array 性質个
table 就有可能重複既有个 item，我共伊改過來：
