...
- [X] 主題切換：md 用 tokyonight, 其他 用 night-owl - 勉強過關
- [X] 試用 astroui folding - 會用得个時勉強過關，但是定定失靈
- [X] 合併 .zprofile 佮 .zshrc
- [X] 比較 heirline 佮 lualine
- [X] 改用 AstroNvim 做底
- [X] AstroNvim + render-markdown + tokyonight + styler 初步成功

NOTE: 今仔日个感想：五色令人目盲，畫面創甲傷花，傷目睭、傷元氣，毋值。
render-markdown 設定規工，嘛袂凍全然滿意，暫停也好。folding 總算設到會用得个
程度。

## MD 主題用 tokyonight，其他用 night-owl
...
NOTE: 切換了應該是有成功，不而過一旦叫入 MD 檔了後，閣切過 lua 檔， `:colorscheme`
猶原會一直顯示是 tokyonight。

### folke/styler.nvim 解法

佇 init.lua 指定主要主題 night-owl，另外佇 plugins/styler.lua 指定 MD 所用个主題
tokyonight-night。試了敢哪有成功，但是`:colorscheme`會攏變做`default`。

styler.nivm 有提供`:Styler`指令通手動切換主題，雖然換會過，但是`:colorscheme`
仝款攏顯示`default`。但是 Styler 會使針對每一个 buffer 設定無仝主題，所以毋是
colorscheme 个同義詞。
...
` 來用。
2. 愛閣 copy `_astrotheme`。

## 合併 .zprofile 佮 .zshrc

叫 Warp AI 將 .zprofile 盡量清予空，一寡設定攏集中來 .zshrc 管理。
...
- **Stars**: 7,239 ⭐ (significantly more popular)
...
- **Stars**: 1,167 ⭐ (smaller but dedicated community)
...
- ✅ Easy theming with built-in themes
- ✅ Pre-made components (git branch, diagnostics, etc.)
- ✅ Simple section-based layout
- ❌ Limited layout flexibility
- ❌ Harder to create completely custom components

**heirline.nvim**:

- ✅ Complete control over every pixel
- ✅ Dynamic layouts and conditional rendering
- ✅ Can create statusline, winbar, tabline all with one plugin
- ✅ Extremely efficient component system
- ❌ Steeper learning curve
- ❌ No built-in themes (you build everything yourself)
...
因為 folding 一直設袂周至，即馬改用 AstroNvim 做基礎，tha̍h我个核心設定佮插件，
同時也去捒一寡 AstroNvim 个家婆設定，親像 MD 个自動完成功能（補充：用 toggle
keymap 就好）。

經過濟濟擺試用閣試用，今閣翻頭來用 AstroNvim，加足有概念矣！主題換過來 night-owl，
comment 色水改較顯目，其他个 highlight 設定也攏佇 astroui.lua 內底做。
自動完成功能會使 `<leader>uc` 禁khài。我家己个三大設定檔輕鬆引進入來，干焦修改
keymaps.lua 內底 require("Comment.api") 進前先 pcall 確認。

完成度真懸个 folding 拗逝設定是我上大个收益！將一大堆 AI 回應攏收起來，按呢來編寫
學習日誌才會輕可啦！

AstroNvim 會記得頂擺个游標位置！伊也有提供 Zen mode，減少視覺轟炸。

AstroNvim + render-markdown + tokyonight theme + styler 个組合運用，初步成功！
指令 `RenderMarkdown buf_disable` 佮 `RenderMarkdown buf_enable` 就會使關／開
目前个buffer矣，當然也有 global 个 disable/enable 版。
